package tr.theyusa.v4war.fmt.shadowsocks
...
import tr.theyusa.v4war.fmt.parseBoxUot
...
import tr.theyusa.v4war.ktx.substringBetween
...
fun JSONMap.parseShadowsocks(): ShadowsocksBean {
    return ShadowsocksBean().apply {
        ...
        serverPort = getIntOrNull("server_port") ?: defaultPort
        password = getStr("password").orEmpty()
        method = getStr("method").orEmpty()
        name = (this@parseShadowsocks["remarks"] as? String).orEmpty()

        val pId = getStr("plugin")
        if (!pId.isNullOrBlank()) {
            plugin = pId + ";" + (this@parseShadowsocks["plugin_opts"] as? String).orEmpty()
            pluginToLocal()
        }
    }
}
